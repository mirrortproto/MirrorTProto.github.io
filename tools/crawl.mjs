...
const langCodes = (html) => new Set([...html.matchAll(/setln=([a-z-]{2,5})\b/g)].map((m) => m[1]));
...
const DE_WORDS = /\b(und|der|die|das|für|von|nicht|oder|mit|sich|auch|Sie|wir|werden)\b/g;
function looksEnglish(html) {
  const text = html.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<[^>]+>/g, ' ');
  return (text.match(EN_WORDS) || []).length >= (text.match(DE_WORDS) || []).length;
}

async function fetchUrl(url, tries = 3) {
...
const pageUrl = (origin, p) => origin + p.split('/').map(encodeURIComponent).join('/');

async function fetchPage(p) {
  return (await fetchUrl(pageUrl(BASE, p))).html;
}
...
const titleOf = (html) => ((html.match(/<title>([^<]*)</) || [])[1] || '').trim();
const catchAllTitles = new Map();
async function catchAllTitle(origin) {
...
async function fetchDocument(u) {
...
async function main() {
  ...
  const seen = new Set();
  const titles = new Map(); // file -> its <title>, for the stub audit below
  const fileOwner = new Map(); // file name -> the host whose page it holds
  const queue = [...SEEDS];
  const manifest = [];
  let fetched = 0;
  let cached = 0;
  ...
  const extra = JSON.parse(await readFile(path.join(ROOT, 'tools', 'extra-pages.json'), 'utf8'));
  ...
  const prev = await readFile(path.join(BACKUP, 'manifest.json'), 'utf8')
    .then(JSON.parse)
    .catch(() => ({}));
  const knownRedirect = new Map((prev.redirects || []).map((r) => [r.url, r]));
  const knownSkip = { ...(prev.not_mirrored || {}) };
  const redirects = [];
  const skipped = { ...extra.not_mirrored };
  const translated = { ...(prev.translated || {}) };
  const seenKeys = new Set(); // host+path of every extra page already looked at
  const wanted = new Map(); // path -> section group
  ...
  const isTranslationPath = (p) => stripLang(p) !== p;
  ...
  for (const [group, urls] of Object.entries(extra.groups)) {
    for (const u of urls) {
      ...
      if (knownRedirect.has(u)) {
        redirects.push(knownRedirect.get(u));
        continue;
      }
      if (knownSkip[u]) {
      ...
      let file = safeName(p);
      if (fileOwner.has(file) && fileOwner.get(file) !== host) file = host + '__' + file;
      ...
      try {
        ...
        try {
          ...
          skipped[u] = `не скачалось: ${e.message}`;
          continue;
        }
        ...
        if (got.translation) {
          skipped[u] = `оригинал отдаёт только перевод (${got.translation}) — зеркало английское`;
          continue;
        }
        const stand = await catchAllTitle(asked.origin);
        if (stand && titleOf(got.html) === stand) {
          skipped[u] = `оригинал отдаёт по этому адресу свою заглушку «${stand}» — страницы нет`;
          continue;
        }
        html = got.html;
        await writeFile(full, html);
      ...
      if (finalUrl !== u && finalUrl !== asked.origin + p) entry.final_url = finalUrl;
      manifest.push(entry);
    }
  }
  ...
  try {
    const prev = JSON.parse(await readFile(path.join(BACKUP, 'manifest.json'), 'utf8'));
    ...
    const excused = new Set([
      ...Object.keys(skipped).map((u) => decodeURIComponent(new URL(u).pathname).replace(/\/+$/, '')),
      ...redirects.map((r) => r.path),
    ]);
    const gone = prev.pages.map((p) => p.path).filter((p) => !now.has(p) && !excused.has(p));
    if (gone.length) {
      console.error('FAIL: pages present in the previous manifest and missing now:');
  ...
  if (stubPages.length) {
    ...
    for (const e of stubPages.filter((e) => e.group)) {
      skipped[e.url] = `оригинал отдаёт по этому адресу свою заглушку «${await catchAllTitle('https://' + new URL(e.url).hostname)}» — страницы нет`;
    ...
    if (fromSections.length) {
      console.error('FAIL: the documentation crawl stored the "not found" page of the original:');
      for (const e of fromSections) console.error('  ' + e.url + ' → ' + e.file);
      console.error('Delete those files and re-run: a section page that answers with a stub is a bug in the crawl, not a fact about the page.');
      process.exit(1);
    }
  }
  ...
  for (const r of staleAliases) skipped[r.url] = `302 → ${r.to}, которой нет в зеркале`;
  ...
  const meta = {
    ...
    sections: [
      ...
      'Blog (/blog)',
      'FAQ (/faq, /techfaq, …)',
      'Other (linked pages of core.telegram.org and telegram.org)',
    ],
    date,
  ...
  console.log('redirect aliases:', liveRedirects.length, '| not mirrored:', Object.keys(skipped).length);
}
