...
//
//   node tools/extlinks.mjs            → external-links.md + external-links.json
//   node tools/extlinks.mjs --out DIR  → то же самое в другом каталоге
...
for (const f of files) {
  ...
  const src = note < 0 ? null : /href="(https?:\/\/[^"]+)"/.exec(html.slice(note));
  mirrored.set(urlOf(f).replace(/\/$/, ''), src ? new URL(src[1]).hostname.replace(/^www\./, '') : null);
}
...
for (const file of files) {
  ...
  while ((m = A_RE.exec(body))) {
    ...
    if (!/^https?:\/\//i.test(raw)) continue; // внутренние проверяет tools/verify.mjs
    let u;
    try {
      u = new URL(raw);
    } catch {
    ...
    t.schemes.add(u.protocol.replace(':', ''));
    t.refs++;
    if (!t.pages.has(page)) t.pages.set(page, []);
    t.pages.get(page).push({ frag: u.hash || '', text: unTag(m[2]) });
  }
}

// ---- классификация ----
const TG_HOSTS = new Set(['core.telegram.org', 'telegram.org', 'www.telegram.org']);
const isAsset = (t) =>
...
const groups = {
  ...
  layer: { title: 'Версии страниц зеркала (`?layer=N`) — намеренно наружу', items: [] },
  asset: { title: 'Файлы и медиа оригинала (не страницы)', items: [] },
  third: { title: 'Сторонние сайты', items: [] },
  anomaly: { title: 'АНОМАЛИИ: ссылка уходит наружу, хотя страница в зеркале есть', items: [] },
};
...
for (const t of targets.values()) {
  ...
  t.mirroredAs =
    TG_HOSTS.has(t.host) && !t.query && t.path !== '/' && mirrored.get(t.path) === t.host
      ? t.path + '/'
      : null;
...
// tools/extract.mjs) — это не дефект локализации.
const idsCache = new Map();
const idsOf = (sitePath) => {
  if (idsCache.has(sitePath)) return idsCache.get(sitePath);
...
for (const t of groups.anomaly.items) {
  const why = [];
  const frags = new Set();
  for (const hits of t.pages.values()) for (const h of hits) if (h.frag) frags.add(h.frag.slice(1));
  const ids = idsOf(t.mirroredAs);
  const dead = [...frags].filter((f) => !ids.has(f) && !ids.has(f.replace(/^q-/, '')));
  const plain = t.refs - [...t.pages.values()].flat().filter((h) => h.frag && dead.includes(h.frag.slice(1))).length;

  if (t.malformed)
    why.push('ДЕФЕКТ: `//host/path` из оригинала склеена с origin страницы (правило cross-page relative links в `tools/extract.mjs` исключает `http(s):` и `#`, но не `//`)');
  if (t.schemes.has('http'))
    why.push('ДЕФЕКТ: локализация матчит только `https://core.telegram.org`, схема `http:` не переписывается');
  if (t.host !== 'core.telegram.org' && !t.malformed)
    why.push('ДЕФЕКТ: по абсолютному URL локализуются только ссылки на `core.telegram.org`, а эта страница зазеркалена с `telegram.org`');
  if (dead.length)
    why.push(`BY DESIGN: якоря ${dead.map((f) => '`#' + f + '`').join(' ')} в зеркале не существуют (оригинал их переименовал) → ссылка оставлена внешней`);
  if (t.mirroredAs === '/')
    why.push('СПОРНО: корень зеркала — собственная страница проекта, а не копия главной core.telegram.org');
  if (!why.length && plain > 0) why.push('ДЕФЕКТ: причина не установлена');
  t.why = why;
}

const byRefs = (a, b) => b.refs - a.refs || a.url.localeCompare(b.url);
for (const g of Object.values(groups)) g.items.sort(byRefs);

// ---- вывод ----
const scheme = (t) => (t.schemes.has('http') ? (t.schemes.size > 1 ? 'http+https' : 'http') : 'https');
const fullUrl = (t) => `${t.schemes.has('https') ? 'https' : 'http'}://${t.url}`;
...
const entry = (t) => {
  ...
  if (t.malformed) flags.push('**битый URL** (протокол-относительная ссылка склеена с origin страницы)');
  out.push(`${t.refs} ссылок · ${t.pages.size} страниц${flags.length ? ' · ' + flags.join(' · ') : ''}`);
  out.push('');
  if (t.why) for (const w of t.why) out.push(`> ${w}`);
  if (t.why && t.why.length) out.push('');
  for (const [page, hits] of [...t.pages].sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0]))) {
    const frags = [...new Set(hits.map((h) => h.frag).filter(Boolean))];
    const texts = [...new Set(hits.map((h) => h.text).filter(Boolean))].slice(0, 4);
    const detail = [
      ...
      .filter(Boolean)
      .join(' — ');
    out.push(`- \`${page}\`${hits.length > 1 ? ` ×${hits.length}` : ''}${detail ? ' — ' + detail : ''}`);
  }
  out.push('');
  return out.join('\n');
};
...
md.push(
  `Сгенерировано \`tools/extlinks.mjs\` по каталогу \`${ROOT}/\`: ${files.length} страниц, ` +
...
md.push('Внутренние ссылки здесь не разбираются — их полностью проверяет `npm run check` (`tools/verify.mjs`).\n');
md.push('## Сводка\n');
md.push('| Категория | Адресов | Ссылок |');
md.push('|---|---:|---:|');
for (const g of Object.values(groups)) {
  md.push(`| ${g.title} | ${g.items.length} | ${g.items.reduce((s, t) => s + t.refs, 0)} |`);
}
md.push('');
...
for (const [key, g] of Object.entries(groups)) {
  ...
  if (key === 'third') {
    ...
    for (const t of g.items) {
      if (!hosts.has(t.host)) hosts.set(t.host, []);
      hosts.get(t.host).push(t);
    }
    for (const [host, items] of [...hosts].sort(
      (a, b) => b[1].reduce((s, t) => s + t.refs, 0) - a[1].reduce((s, t) => s + t.refs, 0)
    )) {
      md.push(`### ${host} — ${items.length} адресов, ${items.reduce((s, t) => s + t.refs, 0)} ссылок\n`);
...
console.log(`${mdPath}   ${targets.size} адресов / ${total} ссылок на ${files.length} страницах`);
console.log(`${jsonPath}`);
for (const g of Object.values(groups)) {
  console.log(`  ${String(g.items.length).padStart(4)} адресов, ${String(g.items.reduce((s, t) => s + t.refs, 0)).padStart(4)} ссылок — ${g.title}`);
}
