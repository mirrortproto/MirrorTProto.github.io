...
// over its own links.
//
//   node tools/extlinks.mjs        # what the built site links to right now
//   node tools/expand-extra.mjs    # queue the missing pages for the next crawl
...
const FILE_EXT = /\.(png|jpe?g|gif|svg|ico|webp|mp4|webm|zip|tgz|tar|pdf|xml|txt|css|js)$/i;
...
const groupFor = (host, p) => {
  if (/(^|\/)(faq|techfaq)([_/]|$)/.test(p) || /faq_[a-z]+$/.test(p)) return 'FAQ';
  if (p === '/blog' || p.startsWith('/blog/')) return 'Blog';
  if (host === 'core.telegram.org' && (p === '/bots' || p.startsWith('/bots/'))) return 'Bot API';
  return 'Other';
};
...
const held = new Set(manifest.pages.map((p) => new URL(p.url).hostname + p.path));
...
const candidates = new Map(); // url -> refs
for (const g of Object.values(report.groups)) {
  for (const item of g.items) {
    ...
    if (held.has(u.hostname + p) || ruledOut.has(url) || ruledOut.has(item.url) || queued.has(url)) continue;
...
const added = { 'Bot API': [], Blog: [], FAQ: [], Other: [] };
...
for (const [group, urls] of Object.entries(added)) {
  ...
  extra.groups[group] = [...new Set([...(extra.groups[group] || []), ...urls])].sort();
}
...
console.log('extra-pages.json now lists', Object.values(extra.groups).reduce((s, a) => s + a.length, 0), 'pages');
